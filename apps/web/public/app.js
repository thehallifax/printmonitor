...
function compactCardFor(printer) {
  ...
  const target = printer.identity.hostname || printer.identity.ip || printer.identity.inventoryId;
  card.setAttribute("aria-label", `Open details for ${printer.identity.displayName || target}, ${pending ? "pending, never collected" : printer.operationalState}`);
  ...
  card.querySelector("h3").textContent = printer.identity.displayName || target;
  ...
  card.querySelector(".compact-target").textContent = printer.identity.resolvedIp || target;
...
async function showDetails(id) {
  ...
  try {
    ...
    const title = element("h2", "", printer.identity.displayName || printer.identity.hostname || printer.identity.ip || printer.identity.inventoryId);
    const pending = printer.operationalState === "pending";
    ...
    const identity = factList([[printer.identity.hostname ? "Hostname" : "IP", printer.identity.hostname || printer.identity.ip], ["Location", printer.identity.location], ["Serial", printer.identity.serialNumber], ["Adapter", printer.provenance?.adapter]]);
