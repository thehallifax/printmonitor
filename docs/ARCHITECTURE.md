...
On Windows, production supervision uses two uniquely named native Task Scheduler tasks. Each task has an at-startup trigger, an independent restart-on-failure policy, an explicit project working directory, and a PowerShell host that appends stdout/stderr under `data\log`. Tasks run as the built-in NetworkService account with a limited token, so no password or SNMP community is embedded in task definitions. Installation grants that identity read/execute access to the checkout and modify access only to configured local data/log paths. Installation and lifecycle mutation require elevation, but the API and collector do not run as LocalSystem. Stopping disables both tasks before terminating them so restart policy cannot immediately relaunch the processes.
