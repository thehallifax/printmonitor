...
The static browser client renders two presentations from the same `/api/fleet` response and ordering: an explanatory Operational grouping and a dense Compact grid. View selection is browser-local preference only. Shared consumable presentation helpers consume normalized percentages without deriving values from raw Printer-MIB evidence: Compact uses K/C/M/Y tiles, while Operational uses horizontal attention rows. Summary controls filter the already-loaded client state; independent stale and low-supply predicates do not change health. Site remains in API/storage contracts but is omitted from the single-site UI. Maintenance and complete raw supply evidence remain separate from the primary toner display.
